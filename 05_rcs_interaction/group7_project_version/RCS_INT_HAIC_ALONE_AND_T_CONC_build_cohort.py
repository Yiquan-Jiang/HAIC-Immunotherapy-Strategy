#!/usr/bin/env python3
"""
RCS 分析队列：main_group 为 HAIC_alone 与 HAIC+T_concurrent（TIDY 插补基线 + TIDY 纵向）。

数据源（脚本同目录）:
  - HAIC_NO_TACE_4_TIDY_baseline_imputed.csv
  - HAIC_NO_TACE_4_TIDY_longitudinal.csv  （纵向中 timepoint_type=baseline 对应原 pre-HAIC-1）

输出: data/RCS_INT_HAIC_ALONE_AND_T_CONC_cohort.csv

下游: RCS_INT_HAIC_ALONE_AND_T_CONC_dual_timescale.R
"""
...
OUT_PATH = os.path.join(DATA_DIR, "RCS_INT_HAIC_ALONE_AND_T_CONC_cohort.csv")

MAIN_GROUPS = ("HAIC_alone", "HAIC+T_concurrent")
TRT_COL = "trt_haic_t_conc"
TRT_VAL = "HAIC+T_concurrent"
...
print("=" * 70)
print("RCS_INT_HAIC_ALONE_AND_T_CONC — cohort from TIDY baseline (imputed) + longitudinal")
print("=" * 70)
...
print(f"  HAIC_alone: {(full[TRT_COL] == 0).sum()}  |  HAIC+T_concurrent: {full[TRT_COL].sum()}")

LANDMARK_MONTHS = 42 / 30.44
full["os_lm"] = full["os_months"] - LANDMARK_MONTHS
lm = full[full["os_lm"] > 0]
print(f"\nLandmark os_lm>0: {len(lm)}  |  alone: {(lm[TRT_COL]==0).sum()}  |  T_conc: {lm[TRT_COL].sum()}")
print("Done.")