#!/usr/bin/env Rscript
# =============================================================================
# RCS_INT_HAIC_ALONE_AND_THEN_I — rms::rcs() + IPTW 加权 rms::cph()，双时间尺度
#
# 队列: TIDY main_group 为 HAIC_alone vs HAIC_then_I；暴露项
#   Surv(...) ~ trt_haic_then_i * rcs(rcsx, nk)
#
# 依赖: survival, rms, Hmisc, ggplot2, dplyr, glmnet, gridExtra, grid
#
# 数据: RCS_INT_HAIC_ALONE_AND_THEN_I_cohort.csv（先运行 RCS_INT_HAIC_ALONE_AND_THEN_I_build_cohort.py）
# 输出: output/RCS_INT_HAIC_ALONE_AND_THEN_I/{landmark,total_os}/
...
DATA_CSV <- Sys.getenv(
  "RMS_RCS_DATA_CSV",
  unset = normalizePath(file.path(SCRIPT_DIR, "data", "RCS_INT_HAIC_ALONE_AND_THEN_I_cohort.csv"), mustWork = FALSE)
)
BASE_OUT <- Sys.getenv(
  "RMS_RCS_OUT_DIR",
  unset = file.path(SCRIPT_DIR, "output", "RCS_INT_HAIC_ALONE_AND_THEN_I")
)
...
# 治疗变量列名与标签（可通过环境变量覆盖，默认 HAIC_alone vs HAIC_then_I）
TRT_COL   <- Sys.getenv("RMS_RCS_TRT_COL",   unset = "trt_haic_then_i")
TRT_LABEL <- Sys.getenv("RMS_RCS_TRT_LABEL", unset = "HAIC_then_I")
