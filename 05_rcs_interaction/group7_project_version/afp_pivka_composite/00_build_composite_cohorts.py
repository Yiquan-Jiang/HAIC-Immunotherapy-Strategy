...
COHORT_FILES = {
    "THEN_IT": {
        "csv": "RCS_INT_HAIC_ALONE_AND_THEN_IT_cohort.csv",
        "trt_col": "trt_haic_then_it",
    },
    "THEN_I": {
        "csv": "RCS_INT_HAIC_ALONE_AND_THEN_I_cohort.csv",
        "trt_col": "trt_haic_then_i",
    },
    "THEN_T": {
        "csv": "RCS_INT_HAIC_ALONE_AND_THEN_T_cohort.csv",
        "trt_col": "trt_haic_then_t",
    },
    "T_CONC": {
        "csv": "RCS_INT_HAIC_ALONE_AND_T_CONC_cohort.csv",
        "trt_col": "trt_haic_t_conc",
    },
    "I_CONC": {
        "csv": "RCS_INT_HAIC_ALONE_AND_I_CONC_cohort.csv",
        "trt_col": "trt_haic_i_conc",
    },
    "I_T_CONC": {
        "csv": "RCS_INT_HAIC_ALONE_AND_I_T_CONC_cohort.csv",
...
def process_one_cohort(key: str, info: dict):
    src = os.path.join(PARENT_DIR, "data", info["csv"])
    ...
    df = pd.read_csv(src)
    print(f"\n{'='*60}")
    print(f"[{key}] {info['csv']}  n={len(df)}  trt_col={info['trt_col']}")
    print(f"{'='*60}")
