#!/usr/bin/env python3
"""
RCS 分析队列：main_group 为 HAIC_alone 与 HAIC_then_I（TIDY 插补基线 + TIDY 纵向）。

数据源（脚本同目录）:
  - HAIC_NO_TACE_4_TIDY_baseline_imputed.csv
  - HAIC_NO_TACE_4_TIDY_longitudinal.csv  （纵向中 timepoint_type=baseline 对应原 pre-HAIC-1）

输出: RCS_INT_HAIC_ALONE_AND_THEN_I_cohort.csv

下游: RCS_INT_HAIC_ALONE_AND_THEN_I_dual_timescale.R
"""
...
OUT_PATH = os.path.join(DATA_DIR, "RCS_INT_HAIC_ALONE_AND_THEN_I_cohort.csv")

MAIN_GROUPS = ("HAIC_alone", "HAIC_then_I")
...
print("=" * 70)
print("RCS_INT_HAIC_ALONE_AND_THEN_I — cohort from TIDY baseline (imputed) + longitudinal")
print("=" * 70)
...
bl["trt_haic_then_i"] = (bl["main_group"] == "HAIC_then_I").astype(int)
...
# Johnson et al. ALBI：log10(总胆红素 μmol/L)×0.66 − 白蛋白(g/L)×0.085
# TIDY 列 albi_score 与 tbil+alb 按上式重算相差 ≈常数 0.814，等价于对胆红素用了 log10(mg/dL)
# （0.66×log10(17.104)）；分析中勿直接采用 albi_score，以免与文献分界（-2.60 / -1.39）错位。
...
print(f"  HAIC_alone: {(full['trt_haic_then_i'] == 0).sum()}  |  HAIC_then_I: {full['trt_haic_then_i'].sum()}")

LANDMARK_MONTHS = 42 / 30.44
full["os_lm"] = full["os_months"] - LANDMARK_MONTHS
lm = full[full["os_lm"] > 0]
print(f"\nLandmark os_lm>0: {len(lm)}  |  alone: {(lm['trt_haic_then_i']==0).sum()}  |  then_I: {lm['trt_haic_then_i'].sum()}")
print("Done.")
