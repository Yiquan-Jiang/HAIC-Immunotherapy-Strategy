#!/usr/bin/env Rscript
# =============================================================================
# RCS_INT_HAIC_ALONE_AND_THEN_IT — rms::rcs() + IPTW 加权 rms::cph()，双时间尺度
#
# 队列: TIDY main_group 为 HAIC_alone vs HAIC_then_I+T；暴露项
#   Surv(...) ~ trt_haic_then_it * rcs(rcsx, nk)
#
# 依赖: survival, rms, Hmisc, ggplot2, dplyr, glmnet, gridExtra, grid
#
# 数据: RCS_INT_HAIC_ALONE_AND_THEN_IT_cohort.csv（先运行 RCS_INT_HAIC_ALONE_AND_THEN_IT_build_cohort.py）
# 输出: output/RCS_INT_HAIC_ALONE_AND_THEN_IT/{landmark,total_os}/
...
SCRIPT_DIR <- Sys.getenv("RMS_RCS_SCRIPT_DIR", unset = "")
...
DATA_CSV <- Sys.getenv(
  "RMS_RCS_DATA_CSV",
  unset = normalizePath(file.path(SCRIPT_DIR, "data", "RCS_INT_HAIC_ALONE_AND_THEN_IT_cohort.csv"), mustWork = FALSE)
)
BASE_OUT <- Sys.getenv(
  "RMS_RCS_OUT_DIR",
  unset = file.path(SCRIPT_DIR, "output", "RCS_INT_HAIC_ALONE_AND_THEN_IT")
)
dir.create(BASE_OUT, showWarnings = FALSE, recursive = TRUE)
BASE_OUT <- normalizePath(BASE_OUT, winslash = "/", mustWork = TRUE)

# 治疗变量列名与标签（可通过环境变量覆盖，默认 HAIC_alone vs HAIC_then_I+T）
TRT_COL   <- Sys.getenv("RMS_RCS_TRT_COL",   unset = "trt_haic_then_it")
TRT_LABEL <- Sys.getenv("RMS_RCS_TRT_LABEL", unset = "HAIC_then_I+T")
