#!/usr/bin/env Rscript
# =============================================================================
# RCS_INT_HAIC_ALONE_AND_T_CONC — rms::rcs() + IPTW 加权 rms::cph()，双时间尺度
#
# 队列: TIDY main_group 为 HAIC_alone vs HAIC+T_concurrent；暴露项
#   Surv(...) ~ trt_haic_t_conc * rcs(rcsx, nk)
#
# 依赖: survival, rms, Hmisc, ggplot2, dplyr, glmnet, gridExtra, grid
#
# 数据: RCS_INT_HAIC_ALONE_AND_T_CONC_cohort.csv（先运行 RCS_INT_HAIC_ALONE_AND_T_CONC_build_cohort.py）
# 输出: output/RCS_INT_HAIC_ALONE_AND_T_CONC/{landmark,total_os}/
...
DATA_CSV <- Sys.getenv(
  "RMS_RCS_DATA_CSV",
  unset = normalizePath(file.path(SCRIPT_DIR, "data", "RCS_INT_HAIC_ALONE_AND_T_CONC_cohort.csv"), mustWork = FALSE)
)
BASE_OUT <- Sys.getenv(
  "RMS_RCS_OUT_DIR",
  unset = file.path(SCRIPT_DIR, "output", "RCS_INT_HAIC_ALONE_AND_T_CONC")
)
...
# 治疗变量列名与标签（可通过环境变量覆盖，默认 HAIC_alone vs HAIC+T_concurrent）
TRT_COL   <- Sys.getenv("RMS_RCS_TRT_COL",   unset = "trt_haic_t_conc")
TRT_LABEL <- Sys.getenv("RMS_RCS_TRT_LABEL", unset = "HAIC+T_concurrent")
