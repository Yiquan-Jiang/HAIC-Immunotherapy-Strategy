#!/usr/bin/env python3
"""
RCS 分析队列：main_group 为 HAIC_alone 与 HAIC_then_I+T（TIDY 插补基线 + TIDY 纵向）。

数据源（脚本同目录）:
  - HAIC_NO_TACE_4_TIDY_baseline_imputed.csv
  - HAIC_NO_TACE_4_TIDY_longitudinal.csv  （纵向中 timepoint_type=baseline 对应原 pre-HAIC-1）

输出: RCS_INT_HAIC_ALONE_AND_THEN_IT_cohort.csv

下游: RCS_INT_HAIC_ALONE_AND_THEN_IT_dual_timescale.R
"""
...
OUT_PATH = os.path.join(DATA_DIR, "RCS_INT_HAIC_ALONE_AND_THEN_IT_cohort.csv")

MAIN_GROUPS = ("HAIC_alone", "HAIC_then_I+T")
...
print("=" * 70)
print("RCS_INT_HAIC_ALONE_AND_THEN_IT — cohort from TIDY baseline (imputed) + longitudinal")
print("=" * 70)
...
bl["trt_haic_then_it"] = (bl["main_group"] == "HAIC_then_I+T").astype(int)
...
print(f"  HAIC_alone: {(full['trt_haic_then_it'] == 0).sum()}  |  HAIC_then_I+T: {full['trt_haic_then_it'].sum()}")

LANDMARK_MONTHS = 42 / 30.44
full["os_lm"] = full["os_months"] - LANDMARK_MONTHS
lm = full[full["os_lm"] > 0]
print(f"\nLandmark os_lm>0: {len(lm)}  |  alone: {(lm['trt_haic_then_it']==0).sum()}  |  then_IT: {lm['trt_haic_then_it'].sum()}")
print("Done.")
