#!/usr/bin/env python3
"""
为 PSM 队列 CSV 提取 pre-IT 实验室数据。

复用 afp_pivka_composite/00a_extract_pre_it_labs.py 的逻辑，
针对 cohort_0X_*.csv 文件。

规则：
  - 序贯组（HAIC_then_*）trt=1：取 first_it_date 前 30 天内最近的检查
  - 并发组（HAIC+*_concurrent）trt=1：取 first_haic_date 前 30 天内最近的检查（= baseline）
  - HAIC_alone trt=0：用所有时间点的平均值
...
COHORT_FILES = {
    "01": {"csv": "cohort_01_HAIC_alone_vs_HAIC_I_conc.csv", "type": "concurrent"},
    "02": {"csv": "cohort_02_HAIC_alone_vs_HAIC_then_I.csv", "type": "sequential"},
    "03": {"csv": "cohort_03_HAIC_alone_vs_HAIC_T_conc.csv", "type": "concurrent"},
    "04": {"csv": "cohort_04_HAIC_alone_vs_HAIC_then_T.csv", "type": "sequential"},
    "05": {"csv": "cohort_05_HAIC_alone_vs_HAIC_IT_conc.csv", "type": "concurrent"},
    "06": {"csv": "cohort_06_HAIC_alone_vs_HAIC_then_IT.csv", "type": "sequential"},
}
...
def load_all_lab_data(target_patients):
    ...
    print("  - 肿瘤标志物 (AFP)...")
    tumor = load_lab_data(RAW_TUMOR, ["甲胎蛋白"], ["afp"], target_patients)
    all_lab.update(tumor)

    print("  - 肿瘤标志物 (PIVKA)...")
    pivka = load_lab_data(RAW_PIVKA, ["异常凝血酶原"], ["pivka"], target_patients)
    all_lab.update(pivka)

    for name, lab_df in all_lab.items():
        print(
            f"    {name}: {len(lab_df)} records, {lab_df['patient_id'].nunique()} patients"
        )
...
def extract_pre_it_for_patient(patient_id, target_date, all_lab, lab_cols):
    ...
    for col in lab_cols:
        ...
        val, dt = find_nearest_before(
            patient_id, target_date, lab_df, PRE_IT_WINDOW_DAYS
        )
...
def main():
    print("=" * 70)
    print("Pre-IT Lab Value Extraction for PSM Cohorts")
    print("=" * 70)
    ...
    it_events = sw[
        sw["treatment_category"].isin(["Immunotherapy", "Targeted Therapy"])
    ]
    first_it = it_events.groupby("patient_id")["start_date"].min().reset_index()
    ...
    first_haic = haic_events.groupby("patient_id")["start_date"].min().reset_index()
    ...
    for key, info in COHORT_FILES.items():
        ...
        drop_cols = [
            c
            for c in df.columns
            if c.endswith("_pre_it")
            or c.endswith("_change_pre_it")
            or c
            in [
                "first_it_date",
                "first_haic_date",
        ...
        for idx, row in df.iterrows():
            ...
            if not is_treatment:
                ...
                if pd.notna(target_date):
                    ...
                    has_any = any(
                        pd.notna(pre_it_row.get(f"{c}_pre_it")) for c in raw_lab_cols
                    )
                    if not has_any:
        ...
        pre_it_cols = [
            c
            for c in df.columns
            if c.endswith("_pre_it") or c.endswith("_change_pre_it")
        ]
    ...
    print("\n\n=== Pre-IT extraction for PSM cohorts completed ===")
    print("Ready for RCS_PSM_dual_timescale.R")
