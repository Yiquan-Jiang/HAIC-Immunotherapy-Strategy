...
"""
============================================================================
PSM matched_ids_06：HAIC alone vs HAIC then I+T — 发表级森林图（Fig4 + Landmark 补充）
============================================================================
队列：仅 matched_ids_06 中患者；数据源自 HAIC_NO_TACE_4_TIDY_baseline（=pre-HAIC-1）与
  longitudinal 的 pre_haic_3；分组 = CSV 的 treatment（0=HAIC alone, 1=HAIC then I+T）。
  非加权 Cox / KM / LRT。
  新增变量：tace_combined（随访期间任意时间点有 HAIC+TACE 或 TACE 记录即为 1）。

本目录数据根 ../../；TACE 事件来源：00_swimmer_plot_events.csv。

产出（输出至 publication_output/psm06_IplusT/）：OUT_PREFIX_Fig4_forest_v4；OUT_PREFIX_SuppFig_lm_forest；OUT_PREFIX_Table_lm_forest.csv。
============================================================================
"""
...
OUT_DIR    = os.path.join(SCRIPT_DIR, 'output', 'psm06_IplusT')
os.makedirs(OUT_DIR, exist_ok=True)

# 匹配ID文件路径
MATCHED_IDS_06_LOCAL = os.path.join(DATA_DIR, 'matched_ids_06_HAIC_alone_vs_HAIC_then_I+T.csv')

# TIDY队列数据路径（使用本目录data文件夹）
TIDY_BASELINE_FOR_PSM_SUPP = [
    os.path.join(DATA_DIR, 'HAIC_NO_TACE_4_TIDY_baseline.csv'),
]
TIDY_LONGITUDINAL_FOR_PSM_SUPP = [
    os.path.join(DATA_DIR, 'HAIC_NO_TACE_4_TIDY_longitudinal.csv'),
]

# Swimmer plot事件数据路径
SWIMMER_EVENTS_PATH = os.path.join(DATA_DIR, '00_swimmer_plot_events.csv')
...
OUT_PREFIX = 'psm06_tidymatched_unweighted_IplusT'
...
print("=" * 70)
print("加载数据 [PSM matched_ids_06 + TIDY baseline≡pre-HAIC-1 + longitudinal]")
print("=" * 70)

mid_path = MATCHED_IDS_06_LOCAL
if not os.path.isfile(mid_path):
    raise FileNotFoundError(
        f"未找到 PSM 匹配 ID 文件。已尝试:\n  {MATCHED_IDS_06_LOCAL}\n"
        "请将 matched_ids_06_HAIC_alone_vs_HAIC_then_I+T.csv 放到 publication_relaxed/ 目录。"
    )
...
if 'treatment' not in mid.columns:
    raise ValueError(
        f"{mid_path} 需含列 treatment（0=HAIC alone, 1=HAIC then I+T）"
    )
mid = mid.copy()
...
print(
    f"PSM + TIDY: {len(trace_planB)} 人（matched_ids_06 全数）\n"
    f"  baseline (=pre-HAIC-1): {tid_path}{_long_msg}\n"
    f"  分组 = treatment：HAIC_then_I+T(1)={int(trace_planB['immune_added'].sum())}，"
    f"HAIC_alone(0)={int((trace_planB['immune_added'] == 0).sum())}\n"
    f"  nlr_change_pre3 非空: {n_pre3}"
)

baseline    = pd.read_excel(os.path.join(DATA_DIR, 'HAIC_IMMUNE_baseline.xlsx'))

CSV_SUPP = os.path.join(DATA_DIR, 'HAIC_NO_TACE_4_TIDY_baseline_imputed.csv')
supp = pd.read_csv(CSV_SUPP)
SUPP_COLS = ['patient_id', 'dbil', 'etiology', 'bclc_stage',
             'inr', 'pt', 'mono', 'hb', 'wbc']
supp_sub = supp[supp['patient_id'].isin(trace_planB['patient_id'])][SUPP_COLS].copy()
supp_sub = supp_sub.rename(columns={
    'inr': 'inr_csv', 'pt': 'pt_csv',
    'mono': 'mono_csv', 'hb': 'hb_csv', 'wbc': 'wbc_csv',
})

baseline = baseline.merge(supp_sub, on='patient_id', how='left')
baseline['inr'] = baseline['inr_csv'].combine_first(baseline['inr'])
baseline['pt']  = baseline['pt_csv'].combine_first(baseline['pt'])
baseline['mono'] = baseline['mono_csv'].combine_first(baseline['mono'])
baseline['hb']   = baseline['hb_csv'].combine_first(baseline['hb'])
baseline['wbc']  = baseline['wbc_csv'].combine_first(baseline['wbc'])
...
# ============================================================================
# Phase 1-2: TIDY 分析表 + Excel baseline 补充列 → full
# ============================================================================
print("\n--- Phase 1-2: 构建分析数据框 ---")
...
_swimmer_path = SWIMMER_EVENTS_PATH
...
_afp = pd.to_numeric(full['afp'], errors='coerce')
full['f4_afp_ge20_bl'] = _f4_bin_ge(_afp, 20)
_pivka = pd.to_numeric(full['pivka'], errors='coerce')
full['f4_pivka_band300_8000'] = np.nan
_m = _pivka.notna()
full.loc[_m, 'f4_pivka_band300_8000'] = ((_pivka[_m] >= 300) & (_pivka[_m] <= 8000)).astype(float)
full['f4_nlr_ge4'] = _f4_bin_ge(full['nlr_bl'], 4.0)
full['f4_albi_gt_m29'] = _f4_bin_gt(full['albi_bl'], -2.9)
_alb = pd.to_numeric(full.get('alb_bl', full.get('alb', pd.Series(np.nan, index=full.index))), errors='coerce')
full['f4_alb_ge40'] = _f4_bin_ge(_alb, 40)
_tbil = pd.to_numeric(full.get('tbil_bl', full.get('tbil', pd.Series(np.nan, index=full.index))), errors='coerce')
full['f4_tbil_ge10'] = _f4_bin_ge(_tbil, 10)
_plr = pd.to_numeric(full['plr_bl'], errors='coerce')
full['f4_plr_band140_210'] = np.nan
_m = _plr.notna()
full.loc[_m, 'f4_plr_band140_210'] = ((_plr[_m] >= 140) & (_plr[_m] <= 210)).astype(float)
_plt = pd.to_numeric(full['plt'], errors='coerce')
_nlr = pd.to_numeric(full['nlr_bl'], errors='coerce')
if 'sii_bl' in full.columns:
    _sii = pd.to_numeric(full['sii_bl'], errors='coerce')
else:
    _sii = _plt * _nlr
full['f4_sii_ge430'] = _f4_bin_ge(_sii, 430)
_mono = pd.to_numeric(full['mono_bl'], errors='coerce')
if 'piv_bl' in full.columns:
    _piv_bl = pd.to_numeric(full['piv_bl'], errors='coerce')
else:
    _piv_bl = _mono * _plt * _nlr
full['f4_piv_ge200_bl'] = _f4_bin_ge(_piv_bl, 200)
full['f4_neut_ge35'] = _f4_bin_ge(full['neut_bl'], 3.5)
full['f4_lymph_ge11'] = _f4_bin_ge(full['lymph_bl'], 1.1)
full['f4_mono_ge045'] = _f4_bin_ge(_mono, 0.45)
full['f4_plt_band160_240'] = np.nan
_m = _plt.notna()
full.loc[_m, 'f4_plt_band160_240'] = ((_plt[_m] >= 160) & (_plt[_m] <= 240)).astype(float)
full['f4_tumor_ge10'] = _f4_bin_ge(full['tumor_max_diameter_cm'], 10)
_afp3 = pd.to_numeric(full.get('afp_pre3'), errors='coerce')
full['f4_afp_ge20_p3'] = _f4_bin_ge(_afp3, 20)
if 'piv_pre3' in full.columns:
    _piv_p3 = pd.to_numeric(full['piv_pre3'], errors='coerce')
else:
    _m3 = pd.to_numeric(full.get('mono_pre3'), errors='coerce')
    _p3 = pd.to_numeric(full.get('plt_pre3'), errors='coerce')
    _n3 = pd.to_numeric(full.get('nlr_pre3'), errors='coerce')
    _piv_p3 = _m3 * _p3 * _n3
full['f4_piv_ge300_p3'] = _f4_bin_ge(_piv_p3, 300)
_ac = pd.to_numeric(full.get('afp_change_pre3'), errors='coerce')
full['f4_dyn_afp_drop_gt90'] = _f4_bin_lt(_ac, -90)
_pc = pd.to_numeric(full.get('piv_change_pre3'), errors='coerce')
full['f4_dyn_piv_rise_gt45'] = _f4_bin_gt(_pc, 45)
...
# 连续亚组：临床固定切分（非 LRT）；中位数分层 / 血常规见 FIG4_SHOW_LEGACY_*（默认关闭）
FIG4_SHOW_LEGACY_INFLAM_MEDIAN = False   # True 时追加 Neut/PLR/Mono 中位数分层
FIG4_SHOW_LEGACY_BLOOD_COUNT = False     # True 时追加 Hb/WBC 中位数分层

subgroups = [
    ('Overall', None, None, True, False, None),
    ('SEP', None, None, False, False, None),
    ('Continuous biomarkers (baseline, fixed cutoffs)', None, None, False, True, None),
    ('  AFP ≥ 20 ng/mL (baseline)', 'f4_afp_ge20_bl', 1, True, False, None),
    ('  AFP < 20 ng/mL (baseline)', 'f4_afp_ge20_bl', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  PIVKA-II 300–8000 mAU/mL', 'f4_pivka_band300_8000', 1, True, False, None),
    ('  PIVKA-II outside 300–8000', 'f4_pivka_band300_8000', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  NLR ≥ 4', 'f4_nlr_ge4', 1, True, False, None),
    ('  NLR < 4', 'f4_nlr_ge4', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  ALBI > −2.9', 'f4_albi_gt_m29', 1, True, False, None),
    ('  ALBI ≤ −2.9', 'f4_albi_gt_m29', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  Albumin ≥ 40 g/L', 'f4_alb_ge40', 1, True, False, None),
    ('  Albumin < 40 g/L', 'f4_alb_ge40', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  TBIL ≥ 10 μmol/L', 'f4_tbil_ge10', 1, True, False, None),
    ('  TBIL < 10 μmol/L', 'f4_tbil_ge10', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  PLR 140–210', 'f4_plr_band140_210', 1, True, False, None),
    ('  PLR outside 140–210', 'f4_plr_band140_210', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  SII (PLT×NLR) ≥ 430', 'f4_sii_ge430', 1, True, False, None),
    ('  SII (PLT×NLR) < 430', 'f4_sii_ge430', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  PIV (Mono×PLT×NLR) ≥ 200', 'f4_piv_ge200_bl', 1, True, False, None),
    ('  PIV (Mono×PLT×NLR) < 200', 'f4_piv_ge200_bl', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  Neutrophil ≥ 3.5', 'f4_neut_ge35', 1, True, False, None),
    ('  Neutrophil < 3.5', 'f4_neut_ge35', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  Lymphocyte ≥ 1.1', 'f4_lymph_ge11', 1, True, False, None),
    ('  Lymphocyte < 1.1', 'f4_lymph_ge11', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  Monocyte ≥ 0.45', 'f4_mono_ge045', 1, True, False, None),
    ('  Monocyte < 0.45', 'f4_mono_ge045', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  Platelet 160–240', 'f4_plt_band160_240', 1, True, False, None),
    ('  Platelet outside 160–240', 'f4_plt_band160_240', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  Tumor diameter ≥ 10 cm', 'f4_tumor_ge10', 1, True, False, None),
    ('  Tumor diameter < 10 cm', 'f4_tumor_ge10', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    # 肝功能（分类，与主分析一致）
    ('Liver function', None, None, False, True, None),
    ('  ALBI Grade 1 (≤ −2.6)', 'albi_good', 1, True, False, None),
    ('  ALBI Grade 2–3 (> −2.6)', 'albi_good', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('Coagulation', None, None, False, True, None),
    (f'  INR low (< median {inr_median:.2f})', 'inr_high', 0, True, False, None),
    (f'  INR high (≥ median {inr_median:.2f})', 'inr_high', 1, True, False, None),
    ('SEP', None, None, False, False, None),
    ('Tumor characteristics', None, None, False, True, None),
    ('  Single tumor', None, None, True, False, None),
    ('  2–3 tumors', None, None, True, False, None),
    ('  Multiple (>3) tumors', None, None, True, False, None),
    ('SEP', None, None, False, False, None),
    ('BCLC stage', None, None, False, True, None),
    ('  BCLC A/B', 'bclc_advanced', 0, True, False, None),
    ('  BCLC C', 'bclc_advanced', 1, True, False, None),
    ('SEP', None, None, False, False, None),
    ('Vascular invasion', None, None, False, True, None),
    ...
    ('SEP', None, None, False, False, None),
    ('Extrahepatic spread & portal hypertension', None, None, False, True, None),
    ...
    ('SEP', None, None, False, False, None),
    ('TACE combination', None, None, False, True, None),
    ('  With TACE (any time)', 'tace_combined', 1, True, False, None),
    ('  Without TACE', 'tace_combined', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('Pre-HAIC-3 (day 42, fixed cutoffs)', None, None, False, True, None),
    ('  AFP ≥ 20 ng/mL (pre-HAIC-3)', 'f4_afp_ge20_p3', 1, True, False, None),
    ('  AFP < 20 ng/mL (pre-HAIC-3)', 'f4_afp_ge20_p3', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  PIV (Mono×PLT×NLR) ≥ 300', 'f4_piv_ge300_p3', 1, True, False, None),
    ('  PIV (Mono×PLT×NLR) < 300', 'f4_piv_ge300_p3', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('Dynamic change (pre-HAIC-3 vs baseline)', None, None, False, True, None),
    ('  AFP decline > 90%', 'f4_dyn_afp_drop_gt90', 1, True, False, None),
    ('  AFP decline ≤ 90%', 'f4_dyn_afp_drop_gt90', 0, True, False, None),
    ('SEP', None, None, False, False, None),
    ('  PIV increase > 45%', 'f4_dyn_piv_rise_gt45', 1, True, False, None),
    ('  PIV increase ≤ 45%', 'f4_dyn_piv_rise_gt45', 0, True, False, None),
]

_LEGACY_FIG4_INFLAM = [
    ('SEP', None, None, False, False, None),
    ('Inflammatory markers (median split, legacy)', None, None, False, True, None),
    (f'  Neutrophil low (< {neut_median:.1f})', 'neut_high', 0, True, False, None),
    (f'  Neutrophil high (≥ {neut_median:.1f})', 'neut_high', 1, True, False, None),
    (f'  PLR low (< {plr_median:.0f})', 'plr_high', 0, True, False, None),
    (f'  PLR high (≥ {plr_median:.0f})', 'plr_high', 1, True, False, None),
    (f'  Monocyte low (< {mono_median:.2f})', 'mono_high', 0, True, False, None),
    (f'  Monocyte high (≥ {mono_median:.2f})', 'mono_high', 1, True, False, None),
]
_LEGACY_FIG4_BLOOD = [
    ('SEP', None, None, False, False, None),
    ('Blood count (median split, legacy)', None, None, False, True, None),
    (f'  Hb high (≥ {hb_median:.0f} g/L)', 'hb_low', 0, True, False, None),
    (f'  Hb low (< {hb_median:.0f} g/L)', 'hb_low', 1, True, False, None),
    (f'  WBC low (< {wbc_median:.1f})', 'wbc_high', 0, True, False, None),
    (f'  WBC high (≥ {wbc_median:.1f})', 'wbc_high', 1, True, False, None),
]
if FIG4_SHOW_LEGACY_INFLAM_MEDIAN:
    subgroups.extend(_LEGACY_FIG4_INFLAM)
if FIG4_SHOW_LEGACY_BLOOD_COUNT:
    subgroups.extend(_LEGACY_FIG4_BLOOD)
...
fig = plt.figure(figsize=(18.0, max(8.0, f4_total_y * 0.24)))
# V4: 新增第7列 P_interaction
gs_f = gridspec.GridSpec(1, 7, figure=fig,
                         width_ratios=[3.0, 0.55, 1.3, 1.5, 1.3, 1.5, 0.9], wspace=0.03)
...
ax_lab.text(0.0, f4_total_y + 0.3, 'Subgroup',    fontsize=LM_FS_HDR, fontweight='bold')
ax_n.text(0.5,   f4_total_y + 0.3, 'I / H',       fontsize=LM_FS_N,   fontweight='bold', ha='center')
ax_hr_txt.text(0.5, f4_total_y + 0.3, 'HR (95% CI)', fontsize=LM_FS_N, fontweight='bold', ha='center')
ax_rmst_txt.text(0.5, f4_total_y + 0.3,
                 f'ΔRMST (95% CI)\n{RMST_FIG4_NOTE}',
                 fontsize=LM_FS_N - 0.5, fontweight='bold', ha='center')
ax_pint.text(0.5, f4_total_y + 0.3, 'P interaction\n(LRT, per stratifier)',
             fontsize=LM_FS_N - 1.0, fontweight='bold', ha='center')
...
ax_hr.set_yticks([])
ax_hr.text(0.5, 1.02, HR_AXIS_LABEL, fontsize=LM_FS_AXIS + 0.5, fontweight='bold',
           ha='center', transform=ax_hr.transAxes, va='bottom')
ax_hr.text(0.02, -0.05, '← Favors\nimmunotherapy', fontsize=LM_FS_AXIS - 1.5, color=CB_GREEN,
           transform=ax_hr.transAxes, ha='left', va='top')
ax_hr.text(0.98, -0.05, 'Favors\nHAIC alone →', fontsize=LM_FS_AXIS - 1.5, color=CB_PINK,
...
ax_rmst.set_yticks([])
ax_rmst.text(0.5, 1.02, 'ΔRMST at 18 months', fontsize=LM_FS_AXIS + 0.5, fontweight='bold',
             ha='center', transform=ax_rmst.transAxes, va='bottom')

# 图例（灰：HR P>0.05 或同配对键内 ΔRMST 较小者，覆盖 P_int 色；其余见下）
f4_legend = [
    mpatches.Patch(color=CB_GRAY,   label='HR P>0.05 or smaller ΔRMST vs paired stratum'),
    mpatches.Patch(color=P_INTERACT_BRIGHT_RED, label='P interaction < 0.05 (if not gray above)'),
    mpatches.Patch(color=P_INTERACT_DARK_RED,   label='0.05 ≤ P interaction < 0.10'),
    mpatches.Patch(color=CB_GREEN,  label='HR < 0.70 (else)'),
    mpatches.Patch(color=CB_LBLUE,  label='Mid HR / mid ΔRMST'),
    mpatches.Patch(color=CB_GRAY,   label='HR > 0.90 (else, not stratum-gray)'),
]
fig.legend(handles=f4_legend, fontsize=LM_FS_P - 0.5, loc='lower center',
           ncol=3, framealpha=0.95, edgecolor='#ccc',
           bbox_to_anchor=(0.5, -0.06),
           handlelength=1.2, handletextpad=0.4, columnspacing=1.0)

_f4_subtitle = (
    'Unweighted HR (robust SE) and unweighted ΔRMST (τ = 18 months); '
    'PSM matched (HAIC alone vs HAIC then I), TIDY baseline≡pre-HAIC-1'
)
fig.suptitle(
    f'Forest Plot: Subgroup Analysis\n{_f4_subtitle}',
    fontsize=13, fontweight='bold', y=1.01
)
# 注脚说明
_f4_foot = (
    'I = HAIC then immunotherapy; H = HAIC alone. '
    'HR: unweighted Cox PH (robust SE). '
    'ΔRMST: unweighted Kaplan-Meier, τ = 18 months. '
    'P interaction: each stratifier × treatment, unweighted LRT (exploratory; no multiplicity adjustment). '
    +
    'Continuous subgroups use fixed clinical cutoffs (no LRT-driven thresholds): '
    'baseline AFP≥20; PIVKA-II 300–8000; NLR≥4; ALBI>−2.9; albumin≥40; TBIL≥10; '
    'PLR 140–210; SII≥430; PIV≥200; neutrophil≥3.5; lymphocyte≥1.1; monocyte≥0.45; '
    'platelet 160–240; tumor≥10 cm; pre-HAIC-3 AFP≥20 and PIV≥300; '
    'dynamic AFP decline >90% and PIV increase >45% vs baseline. '
    'HR & ΔRMST graphics: gray if subgroup HR P>0.05 or if ΔRMST is the smaller within the same stratifier pair (or smallest among 3+ levels, e.g. tumor count); else P interaction colors if applicable; else HR/ΔRMST magnitude colors.'
)
fig.text(0.01, -0.10, _f4_foot, fontsize=7, color='#555', style='italic')
plt.tight_layout(rect=[0, 0.06, 1, 0.98])
plt.savefig(f'{OUT_DIR}/{OUT_PREFIX}_Fig4_forest_v4.pdf', bbox_inches='tight')
plt.savefig(f'{OUT_DIR}/{OUT_PREFIX}_Fig4_forest_v4.png', dpi=300, bbox_inches='tight')
plt.close()
...
print("\n--- Landmark: 固定临床切分（基线 + pre-HAIC-3）---")
lm_fp['opt_afp_bl_high'] = _f4_bin_ge(lm_fp['afp'], 20)
lm_fp['opt_afp_bl_low'] = _f4_bin_lt(lm_fp['afp'], 20)
_piv_bl_lm = pd.to_numeric(lm_fp['pivka'], errors='coerce')
lm_fp['lm_pivka_bl_inband'] = np.nan
_m = _piv_bl_lm.notna()
lm_fp.loc[_m, 'lm_pivka_bl_inband'] = (
    (_piv_bl_lm[_m] >= 300) & (_piv_bl_lm[_m] <= 8000)).astype(float)
lm_fp['opt_nlr_bl_high'] = _f4_bin_ge(lm_fp['nlr_bl'], 4.0)
lm_fp['opt_nlr_bl_low'] = _f4_bin_lt(lm_fp['nlr_bl'], 4.0)
_ablm = pd.to_numeric(lm_fp['albi_bl'], errors='coerce')
lm_fp['opt_albi_bl_poor'] = np.nan
lm_fp['opt_albi_bl_good'] = np.nan
_m = _ablm.notna()
lm_fp.loc[_m, 'opt_albi_bl_poor'] = (_ablm.loc[_m] > -2.9).astype(float)
lm_fp.loc[_m, 'opt_albi_bl_good'] = (_ablm.loc[_m] <= -2.9).astype(float)
lm_fp['opt_tumor_large'] = _f4_bin_ge(lm_fp['tumor_max_diameter_cm'], 10)
lm_fp['opt_tumor_small'] = _f4_bin_lt(lm_fp['tumor_max_diameter_cm'], 10)
_plrbl = pd.to_numeric(lm_fp['plr_bl'], errors='coerce')
lm_fp['lm_plr_bl_inband'] = np.nan
_m = _plrbl.notna()
lm_fp.loc[_m, 'lm_plr_bl_inband'] = (
    (_plrbl[_m] >= 140) & (_plrbl[_m] <= 210)).astype(float)
lm_fp['opt_afp_p3_high'] = _f4_bin_ge(lm_fp['afp_pre3'], 20)
lm_fp['opt_afp_p3_low'] = _f4_bin_lt(lm_fp['afp_pre3'], 20)
if 'piv_pre3' in lm_fp.columns:
    _pivp3 = pd.to_numeric(lm_fp['piv_pre3'], errors='coerce')
else:
    _pivp3 = (
        pd.to_numeric(lm_fp.get('mono_pre3'), errors='coerce')
        * pd.to_numeric(lm_fp.get('plt_pre3'), errors='coerce')
        * pd.to_numeric(lm_fp.get('nlr_pre3'), errors='coerce'))
lm_fp['opt_piv_p3_high'] = _f4_bin_ge(_pivp3, 300)
lm_fp['opt_piv_p3_low'] = _f4_bin_lt(_pivp3, 300)
lm_fp['opt_nlr_p3_high'] = _f4_bin_ge(lm_fp['nlr_pre3'], 4.0)
lm_fp['opt_nlr_p3_low'] = _f4_bin_lt(lm_fp['nlr_pre3'], 4.0)
_a3 = pd.to_numeric(lm_fp['albi_pre3'], errors='coerce')
lm_fp['opt_albi_p3_poor'] = np.nan
lm_fp['opt_albi_p3_good'] = np.nan
_m = _a3.notna()
lm_fp.loc[_m, 'opt_albi_p3_poor'] = (_a3.loc[_m] > -2.9).astype(float)
lm_fp.loc[_m, 'opt_albi_p3_good'] = (_a3.loc[_m] <= -2.9).astype(float)
_plrp3 = pd.to_numeric(lm_fp['plr_pre3'], errors='coerce')
lm_fp['lm_plr_p3_inband'] = np.nan
_m = _plrp3.notna()
lm_fp.loc[_m, 'lm_plr_p3_inband'] = (
    (_plrp3[_m] >= 140) & (_plrp3[_m] <= 210)).astype(float)

# ── 用户制定的动态变量决策规则（固定阈值，基于RCS+因果森林综合判断）──────────────
# NLR:      > -70% → 加免疫（触发）；≤ -70% → 可继续HAIC（豁免）
# AFP:      > -90% → 加免疫（触发）；≤ -90% → 可豁免
# Neut:     > -30% → 加免疫（触发）；≤ -30% → 可豁免
# PLR:      > -40% → 加免疫（触发）；≤ -40% → 可豁免
# PIVKA-II: -88% ~ -20% → 加免疫（中度下降）；< -88% → 获益减弱
# ALBI:     > -13% (稳定/升高) → 加免疫；≤ -13% (明显下降) → 获益减弱
DYN_NLR_EXEMPT  = -70    # NLR 豁免线：降幅 ≥70%
DYN_AFP_EXEMPT  = -90    # AFP 豁免线：降幅 ≥90%
DYN_NEUT_EXEMPT = -30    # Neutrophil 豁免线：降幅 ≥30%
DYN_PLR_EXEMPT  = -40    # PLR 豁免线：降幅 ≥40%
DYN_PIVKA_DEEP  = -88    # PIVKA-II 极深降（获益减弱）
DYN_PIVKA_TRIG  = -20    # PIVKA-II 触发线（中度下降起点）
DYN_ALBI_TRIG   = -13    # ALBI 触发线（明显下降阈值）

# 构建分组列
# ── NLR ──
lm_fp['dyn_nlr_exempt']  = (lm_fp['nlr_change_pre3']   <= DYN_NLR_EXEMPT).astype(float)   # ≤-70%: 豁免
lm_fp['dyn_nlr_trigger'] = (lm_fp['nlr_change_pre3']   >  DYN_NLR_EXEMPT).astype(float)   # >-70%: 触发

# ── AFP ──
lm_fp['dyn_afp_exempt']  = (lm_fp['afp_change_pre3']   <= DYN_AFP_EXEMPT).astype(float)   # ≤-90%: 豁免
lm_fp['dyn_afp_trigger'] = (lm_fp['afp_change_pre3']   >  DYN_AFP_EXEMPT).astype(float)   # >-90%: 触发

# ── Neutrophil ──
lm_fp['dyn_neut_exempt']  = (lm_fp['neut_change_pre3'] <= DYN_NEUT_EXEMPT).astype(float)  # ≤-30%: 豁免
lm_fp['dyn_neut_trigger'] = (lm_fp['neut_change_pre3'] >  DYN_NEUT_EXEMPT).astype(float)  # >-30%: 触发

# ── PLR ──
lm_fp['dyn_plr_exempt']  = (lm_fp['plr_change_pre3']   <= DYN_PLR_EXEMPT).astype(float)   # ≤-40%: 豁免
lm_fp['dyn_plr_trigger'] = (lm_fp['plr_change_pre3']   >  DYN_PLR_EXEMPT).astype(float)   # >-40%: 触发

# ── PIVKA-II ──
lm_fp['dyn_pivka_moderate'] = (                                                             # 中度下降: 触发
    (lm_fp['pivka_change_pre3'] > DYN_PIVKA_DEEP) &
    (lm_fp['pivka_change_pre3'] <= DYN_PIVKA_TRIG)
).astype(float)
lm_fp['dyn_pivka_deep']   = (lm_fp['pivka_change_pre3'] <= DYN_PIVKA_DEEP).astype(float)  # 极深降: 获益减弱

# ── ALBI ──
lm_fp['dyn_albi_stable']  = (lm_fp['albi_change_pre3'] >  DYN_ALBI_TRIG).astype(float)   # >-13%: 触发（稳定/升高）
lm_fp['dyn_albi_improve'] = (lm_fp['albi_change_pre3'] <= DYN_ALBI_TRIG).astype(float)   # ≤-13%: 获益减弱

# ── 用户自定义切割点（固定）────────────────────────────────────────────────
# Baseline: 用户自定义
lm_fp['usr_afp_bl_hi']   = (lm_fp['afp']   >= 1000).astype(float)
lm_fp['usr_afp_bl_lo']   = (lm_fp['afp']   <  1000).astype(float)
lm_fp['usr_pivka_bl_hi'] = (lm_fp['pivka'] >= 8000).astype(float)
lm_fp['usr_pivka_bl_lo'] = (lm_fp['pivka'] <  8000).astype(float)
lm_fp['usr_nlr_bl_hi']   = (lm_fp['nlr_bl'] >= 5).astype(float)
lm_fp['usr_nlr_bl_lo']   = (lm_fp['nlr_bl'] <  5).astype(float)
lm_fp['usr_albi_bl_poor']= (lm_fp['albi_bl'] >= -2.6).astype(float)
lm_fp['usr_albi_bl_good']= (lm_fp['albi_bl'] <  -2.6).astype(float)
lm_fp['usr_tumor_lg']    = (lm_fp['tumor_max_diameter_cm'] >= 10).astype(float)
lm_fp['usr_tumor_sm']    = (lm_fp['tumor_max_diameter_cm'] <  10).astype(float)
# Pre-HAIC-3: 用户自定义
lm_fp['usr_afp_p3_hi']   = (lm_fp['afp_pre3']   >= 1000).astype(float)
lm_fp['usr_afp_p3_lo']   = (lm_fp['afp_pre3']   <  1000).astype(float)
lm_fp['usr_pivka_p3_hi'] = (lm_fp['pivka_pre3'] >= 8000).astype(float)
lm_fp['usr_pivka_p3_lo'] = (lm_fp['pivka_pre3'] <  8000).astype(float)

# 保留旧变量名兼容性
lm_fp['dyn_afp_deep']   = lm_fp['dyn_afp_exempt']
lm_fp['dyn_afp_mid']    = lm_fp['dyn_afp_trigger']
lm_fp['dyn_nlr_deep']   = lm_fp['dyn_nlr_exempt']
lm_fp['dyn_neut_deep']  = lm_fp['dyn_neut_exempt']
lm_fp['dyn_neut_high']  = (lm_fp['neut_change_pre3'] >= 15).astype(float)
lm_fp['dyn_plr_up']     = (lm_fp['plr_change_pre3']  >= 10).astype(float)
lm_fp['dyn_plr_down']   = lm_fp['dyn_plr_trigger']
lm_fp['dyn_pivka_resp'] = lm_fp['dyn_pivka_moderate']
lm_fp['dyn_albi_worse'] = lm_fp['dyn_albi_stable']

# ── pre-haic-3 静态值分组列（绝对值，临床阈值）────────────────────────────────
# AFP pre3: 400 ng/mL
lm_fp['sta_afp_high']    = (lm_fp['afp_pre3']   > 400).astype(float)
lm_fp['sta_afp_low']     = (lm_fp['afp_pre3']   <= 400).astype(float)
# PIVKA pre3: 8000 mAU/mL
lm_fp['sta_pivka_high']  = (lm_fp['pivka_pre3'] > 8000).astype(float)
lm_fp['sta_pivka_low']   = (lm_fp['pivka_pre3'] <= 8000).astype(float)
# NLR pre3: 5
lm_fp['sta_nlr_high']    = (lm_fp['nlr_pre3']   >= 5).astype(float)
lm_fp['sta_nlr_low']     = (lm_fp['nlr_pre3']   < 5).astype(float)
# Neutrophil pre3: 中位数
neut_pre3_median = lm_fp['neut_pre3'].median()
lm_fp['sta_neut_high']   = (lm_fp['neut_pre3']  >= neut_pre3_median).astype(float)
lm_fp['sta_neut_low']    = (lm_fp['neut_pre3']  < neut_pre3_median).astype(float)
# PLR pre3: 中位数
plr_pre3_median = lm_fp['plr_pre3'].median()
lm_fp['sta_plr_high']    = (lm_fp['plr_pre3']   >= plr_pre3_median).astype(float)
lm_fp['sta_plr_low']     = (lm_fp['plr_pre3']   < plr_pre3_median).astype(float)
# ALBI pre3: -2.6 (Grade 1 vs 2-3)
lm_fp['sta_albi_good']   = (lm_fp['albi_pre3']  <= -2.6).astype(float)
lm_fp['sta_albi_poor']   = (lm_fp['albi_pre3']  > -2.6).astype(float)
print(
    f"Landmark 亚组: 连续变量固定切分（Fig4 一致）; pre3 Neut/PLR 中位数分层 — "
    f"AFP≥20 n={lm_fp['opt_afp_p3_high'].sum():.0f}, PIV≥300 n={lm_fp['opt_piv_p3_high'].sum():.0f}, "
    f"Neut med {neut_pre3_median:.1f}, PLR med {plr_pre3_median:.0f}"
)

# 定义亚组（基线 + pre-haic-3 静态 + pre-haic-3 动态）；连续切分均为预设临床阈值
lm_subgroups = [
    # label, col, val, is_calc, is_header
    ('Overall (Landmark, day 42)', None, None, True, False),
    ('SEP', None, None, False, False),
    ('Baseline characteristics', None, None, False, True),
    ('  AFP ≥ 20 ng/mL (baseline)', 'opt_afp_bl_high', 1, True, False),
    ('  AFP < 20 ng/mL (baseline)', 'opt_afp_bl_low', 1, True, False),
    ('SEP', None, None, False, False),
    ('  PIVKA-II 300–8000 mAU/mL (baseline)', 'lm_pivka_bl_inband', 1, True, False),
    ('  PIVKA-II outside 300–8000 (baseline)', 'lm_pivka_bl_inband', 0, True, False),
    ('SEP', None, None, False, False),
    ('  NLR ≥ 4 (baseline)', 'opt_nlr_bl_high', 1, True, False),
    ('  NLR < 4 (baseline)', 'opt_nlr_bl_low', 1, True, False),
    ('  PVTT Vp3–4', 'pvtt_advanced', 1, True, False),
    ('  PVTT absent/Vp1–2', 'pvtt_advanced', 0, True, False),
    ('SEP', None, None, False, False),
    ('  ALBI > −2.9 (poor, baseline)', 'opt_albi_bl_poor', 1, True, False),
    ('  ALBI ≤ −2.9 (good, baseline)', 'opt_albi_bl_good', 1, True, False),
    ('SEP', None, None, False, False),
    ('  Tumor ≥ 10 cm (baseline)', 'opt_tumor_large', 1, True, False),
    ('  Tumor < 10 cm (baseline)', 'opt_tumor_small', 1, True, False),
    ...
    ('SEP', None, None, False, False),
    (f'  Neutrophil high (≥ median {neut_median:.1f}, baseline)', 'neut_high', 1, True, False),
    (f'  Neutrophil low (< median {neut_median:.1f}, baseline)', 'neut_high', 0, True, False),
    ('SEP', None, None, False, False),
    ('  PLR 140–210 (baseline)', 'lm_plr_bl_inband', 1, True, False),
    ('  PLR outside 140–210 (baseline)', 'lm_plr_bl_inband', 0, True, False),
    ('SEP', None, None, False, False),
    ('User-defined baseline cutpoints', None, None, False, True),
    ('  AFP ≥ 1000 ng/mL (baseline, user)', 'usr_afp_bl_hi', 1, True, False),
    ('  AFP < 1000 ng/mL (baseline, user)', 'usr_afp_bl_lo', 1, True, False),
    ('SEP', None, None, False, False),
    ('  PIVKA-II ≥ 8000 (baseline, user)', 'usr_pivka_bl_hi', 1, True, False),
    ('  PIVKA-II < 8000 (baseline, user)', 'usr_pivka_bl_lo', 1, True, False),
    ('SEP', None, None, False, False),
    ('  NLR ≥ 5 (baseline, user)', 'usr_nlr_bl_hi', 1, True, False),
    ('  NLR < 5 (baseline, user)', 'usr_nlr_bl_lo', 1, True, False),
    ('SEP', None, None, False, False),
    ('  ALBI ≥ −2.6 (baseline, user)', 'usr_albi_bl_poor', 1, True, False),
    ('  ALBI < −2.6 (baseline, user)', 'usr_albi_bl_good', 1, True, False),
    ('SEP', None, None, False, False),
    ('  Tumor ≥ 10 cm (baseline, user)', 'usr_tumor_lg', 1, True, False),
    ('  Tumor < 10 cm (baseline, user)', 'usr_tumor_sm', 1, True, False),
    ('SEP', None, None, False, False),
    ('TACE combination', None, None, False, True),
    ('  With TACE (any time)', 'tace_combined', 1, True, False),
    ('  Without TACE', 'tace_combined', 0, True, False),
    ('SEP', None, None, False, False),
    ('Pre-HAIC-3 static values (at cycle 3)', None, None, False, True),
    ('  AFP ≥ 20 ng/mL (pre-HAIC-3)', 'opt_afp_p3_high', 1, True, False),
    ('  AFP < 20 ng/mL (pre-HAIC-3)', 'opt_afp_p3_low', 1, True, False),
    ('SEP', None, None, False, False),
    ('  PIV (Mono×PLT×NLR) ≥ 300 (pre-HAIC-3)', 'opt_piv_p3_high', 1, True, False),
    ('  PIV (Mono×PLT×NLR) < 300 (pre-HAIC-3)', 'opt_piv_p3_low', 1, True, False),
    ('SEP', None, None, False, False),
    ('  NLR ≥ 4 (pre-HAIC-3)', 'opt_nlr_p3_high', 1, True, False),
    ('  NLR < 4 (pre-HAIC-3)', 'opt_nlr_p3_low', 1, True, False),
    ('SEP', None, None, False, False),
    (f'  Neutrophil ≥ {neut_pre3_median:.1f} (pre-HAIC-3, median)', 'sta_neut_high', 1, True, False),
    (f'  Neutrophil < {neut_pre3_median:.1f} (pre-HAIC-3, median)', 'sta_neut_low', 1, True, False),
    ('SEP', None, None, False, False),
    ('  PLR 140–210 (pre-HAIC-3)', 'lm_plr_p3_inband', 1, True, False),
    ('  PLR outside 140–210 (pre-HAIC-3)', 'lm_plr_p3_inband', 0, True, False),
    ('SEP', None, None, False, False),
    ('  ALBI > −2.9 (poor, pre-HAIC-3)', 'opt_albi_p3_poor', 1, True, False),
    ('  ALBI ≤ −2.9 (good, pre-HAIC-3)', 'opt_albi_p3_good', 1, True, False),
    ('SEP', None, None, False, False),
    ('User-defined pre-HAIC-3 cutpoints', None, None, False, True),
    ('  AFP ≥ 1000 ng/mL (pre-HAIC-3, user)', 'usr_afp_p3_hi', 1, True, False),
    ('  AFP < 1000 ng/mL (pre-HAIC-3, user)', 'usr_afp_p3_lo', 1, True, False),
    ('SEP', None, None, False, False),
    ('  PIVKA-II ≥ 8000 (pre-HAIC-3, user)', 'usr_pivka_p3_hi', 1, True, False),
    ('  PIVKA-II < 8000 (pre-HAIC-3, user)', 'usr_pivka_p3_lo', 1, True, False),
    ('SEP', None, None, False, False),
    ('Pre-HAIC-3 dynamic variables (change rate)', None, None, False, True),
    ('  NLR ≤ −70% (exempt: deep drop → HAIC alone)', 'dyn_nlr_exempt', 1, True, False),
    ('  NLR > −70%  (trigger: add immunotherapy)', 'dyn_nlr_trigger', 1, True, False),
    ('SEP', None, None, False, False),
    ('  AFP ≤ −90% (exempt: deep drop → HAIC alone)', 'dyn_afp_exempt', 1, True, False),
    ('  AFP > −90%  (trigger: add immunotherapy)', 'dyn_afp_trigger', 1, True, False),
    ('SEP', None, None, False, False),
    ('  Neutrophil ≤ −30% (exempt: deep drop → HAIC alone)', 'dyn_neut_exempt', 1, True, False),
    ('  Neutrophil > −30%  (trigger: add immunotherapy)', 'dyn_neut_trigger', 1, True, False),
    ('SEP', None, None, False, False),
    ('  PLR ≤ −40% (exempt: deep drop → HAIC alone)', 'dyn_plr_exempt', 1, True, False),
    ('  PLR > −40%  (trigger: add immunotherapy)', 'dyn_plr_trigger', 1, True, False),
    ('SEP', None, None, False, False),
    ('  PIVKA-II −88% ~ −20% (moderate drop → trigger)', 'dyn_pivka_moderate', 1, True, False),
    ('  PIVKA-II < −88% (deep drop → benefit attenuated)', 'dyn_pivka_deep', 1, True, False),
    ('SEP', None, None, False, False),
    ('  ALBI > −13% (stable/rise → trigger)', 'dyn_albi_stable', 1, True, False),
    ('  ALBI ≤ −13% (improve → benefit attenuated)', 'dyn_albi_improve', 1, True, False),
]
...
fig_lm.suptitle(
    ...
    f'Subgroups: Baseline + Pre-HAIC-3 static values + Pre-HAIC-3 dynamic variables\n'
    f'{_lm_st_line}\n'
    f'Continuous subgroups: pre-specified fixed cutpoints (aligned with main Fig4)',
...
print("\n" + "=" * 70)
print(f"完成 [matched_ids_06 HAIC alone vs HAIC then I+T]: {OUT_PREFIX}_Fig4_forest_v4, SuppFig_lm_forest")
