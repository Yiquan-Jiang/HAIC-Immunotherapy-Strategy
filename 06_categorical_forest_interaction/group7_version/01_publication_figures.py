...
def _trace_planb_relaxed_column_order():
    ...
    _cands = [
        os.path.join(DATA_DIR, 'trace_planB_relaxed.csv'),
    ]
    for p in _cands:
        if os.path.isfile(p):
...
plt.savefig(f'{OUT_DIR}/Fig4_forest_v4.pdf', bbox_inches='tight')
plt.savefig(f'{OUT_DIR}/Fig4_forest_v4.png', dpi=300, bbox_inches='tight')
plt.close()
