...
"""
============================================================================
PSM matched_ids_05：HAIC alone vs HAIC+I+T concurrent — 发表级森林图（Fig4 + Landmark 补充）
============================================================================
队列：仅 matched_ids_05 中患者；数据源自 HAIC_NO_TACE_4_TIDY_baseline（=pre-HAIC-1）与
  longitudinal 的 pre_haic_3；分组 = CSV 的 treatment（0=HAIC alone, 1=HAIC+I+T concurrent）。
  非加权 Cox / KM / LRT。
  新增变量：tace_combined（随访期间任意时间点有 HAIC+TACE 或 TACE 记录即为 1）。

本目录数据根 ../../；TACE 事件来源：00_swimmer_plot_events.csv。

产出（输出至 publication_output/psm05_IplusT_concurrent/）：OUT_PREFIX_Fig4_forest_v4；OUT_PREFIX_SuppFig_lm_forest；OUT_PREFIX_Table_lm_forest.csv。
============================================================================
"""
...
OUT_DIR    = os.path.join(PROJECT_ROOT, 'output', 'step2_interaction_forest', 'psm05_HAIC_IplusT_concurrent')
os.makedirs(OUT_DIR, exist_ok=True)

MATCHED_IDS_05_CSV = os.path.normpath(os.path.join(
    DATA_DIR, '..', '..', 'HAIC_NO_TACE_4_TIDY', 'update_group_7', 'results',
    'psm_balance_tables_complete', 'matched_ids_05_HAIC_alone_vs_HAIC+I+T_concurrent.csv'))
MATCHED_IDS_05_LOCAL = os.path.join(SCRIPT_DIR, 'matched_ids_05_HAIC_alone_vs_HAIC+I+T_concurrent.csv')
...
OUT_PREFIX = 'psm05_tidymatched_unweighted_IplusT_concurrent'
...
print("=" * 70)
print("加载数据 [PSM matched_ids_05 + TIDY baseline≡pre-HAIC-1 + longitudinal]")
print("=" * 70)

mid_path = MATCHED_IDS_05_LOCAL
if not os.path.isfile(mid_path):
    mid_path = MATCHED_IDS_05_CSV
if not os.path.isfile(mid_path):
    raise FileNotFoundError(
        f"未找到 PSM 匹配 ID 文件。已尝试:\n  {MATCHED_IDS_05_LOCAL}\n  {MATCHED_IDS_05_CSV}\n"
        "请将 matched_ids_05_HAIC_alone_vs_HAIC+I+T_concurrent.csv 放到 publication_figures/ 目录。"
    )
...
if 'treatment' not in mid.columns:
    raise ValueError(
        f"{mid_path} 需含列 treatment（0=HAIC alone, 1=HAIC+I+T concurrent）"
    )
mid = mid.copy()
...
print(
    f"PSM + TIDY: {len(trace_planB)} 人（matched_ids_05 全数）\n"
    f"  baseline (=pre-HAIC-1): {tid_path}{_long_msg}\n"
    f"  分组 = treatment：HAIC+I+T_concurrent(1)={int(trace_planB['immune_added'].sum())}，"
...
nlr_median  = full['nlr_bl'].median()
albi_median = full['albi_bl'].median()
plt_median  = full['plt'].median()
inr_median  = full['inr'].median() if 'inr' in full.columns else full.get('inr', pd.Series([1.0])).median()
mono_median = full['mono_bl'].median()
lymph_median = full['lymph_bl'].median()
...
full['nlr_high']   = (full['nlr_bl'] >= nlr_median).astype(int)
full['albi_high']  = (full['albi_bl'] >= albi_median).astype(int)
full['plt_high']   = (full['plt'] >= plt_median).astype(int)
full['lymph_high'] = (full['lymph_bl'] >= lymph_median).astype(int)
...
# Legend — 4 categorical colors (gray / 2 reds / favors-treatment)
f4_legend = [
...
plt.savefig(f'{OUT_DIR}/{OUT_PREFIX}_Fig4_forest_v4.pdf', bbox_inches=None)
plt.savefig(f'{OUT_DIR}/{OUT_PREFIX}_Fig4_forest_v4.png', dpi=600, bbox_inches=None)
plt.close()
...
print("\n" + "=" * 70)
print(f"完成 [matched_ids_05 HAIC alone vs HAIC+I+T concurrent]: {OUT_PREFIX}_Fig4_forest_v4, SuppFig_lm_forest")
