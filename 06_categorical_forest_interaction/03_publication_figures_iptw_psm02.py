...
"""
============================================================================
IPTW 敏感性分析：HAIC alone vs HAIC then I — 发表级森林图（Fig4 + Landmark 补充）
============================================================================
队列：原始全队列（非 PSM 匹配），仅筛选 HAIC_alone 与 HAIC_then_I 两组；
  使用 stabilized IPTW 加权平衡基线差异。
  数据源同 01_publication_figures.py：HAIC_NO_TACE_4_TIDY_baseline + longitudinal。
...
OUT_DIR    = os.path.join(PROJECT_ROOT, 'output', 'step2_interaction_forest', 'psm02_HAIC_then_I_IPTW')
...
GROUP_2 = 'HAIC_then_I'      # 治疗组 (treatment=1)

OUT_PREFIX = 'iptw_psm02_HAIC_then_I'
...
fig.suptitle('HAIC then immunotherapy vs HAIC alone — subgroup analysis (IPTW-weighted)',
             fontsize=13, fontweight='bold', y=(1 - 0.30 / _f4_fig_h))

_f4_foot = (
    f'I = {GROUP_2}; H = {GROUP_1}. '
